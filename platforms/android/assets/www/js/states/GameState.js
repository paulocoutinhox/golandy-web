    ...
    GameApp.data.socket.onmessage = function (message) {
        ...
        if (m.type) {
            switch (m.type) {
                ...
                case "player-data":
                    if (context.map == null) {
                        context.map = game.add.tilemap('map' + m.map);
                        ...
                        context.backgroundTileSprite = game.add.tileSprite(bgX, bgY, bgWidth, bgHeight, 'background001');
                    ...
                    context.buttonAddBomb = game.add.button(game.camera.x, game.camera.height - buttonAddBombH, 'buttonAddBomb', function () {
...
GameApp.GameState.prototype.update = function () {
    ...
    if (this.backgroundTileSprite) {
        this.backgroundTileSprite.autoScroll(-GameApp.TILE_SIZE, -GameApp.TILE_SIZE);
    }
...
GameApp.GameState.prototype.addSpawnEffect = function (o, effect) {
    ...
    if (effect == 1) {
        var effect1 = this.groups.underPlayer.create(o.x - 192 / 2 + o.width / 2, o.y - 192 / 2 + 10, 'effectSpritesheet001');
        ...
        var effect2 = this.groups.underPlayer.create(o.x, o.y - 10, 'effectSpritesheet002');
        ...
        var effect3 = this.groups.underPlayer.create(o.x - 192 / 2 + o.width / 2, o.y - 192 / 2 + 10, 'effectSpritesheet003');
...
GameApp.GameState.prototype.playerSetPosition = function (player, x, y, now) {
    ...
    var destinationX = (x * GameApp.TILE_SIZE) + (GameApp.TILE_SIZE / 2);
    var destinationY = (y * GameApp.TILE_SIZE) - (GameApp.TILE_SIZE / 4);
